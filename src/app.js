...
const app = () => {
  ...
  postsContainer.addEventListener('click', (e) => {
    ...
    if (!id) return;
    status.id.add(id);
    const { title, description, link } = status.posts.find((item) => item.id === id) || {};
    if (title) {
  ...
  const updateRSS = () => {
    const promises = status.links.map((link, index) => loadUrl(link)
      .then((response) => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    schema.notOneOf(status.links).validate(link)
      ...
      .then((data) => {
        ...
        status.posts.unshift(...data.posts);
